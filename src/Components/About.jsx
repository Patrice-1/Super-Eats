import React from "react";

function About (){
    return (
      <div className="min-h-screen items-center flex flex-col">
        <h2 className="text-6xl">About SuperEats</h2>
        <p className="text-2xl">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem
          eveniet optio consequatur reprehenderit quisquam voluptatum excepturi,
          rerum dicta tempora, ex ipsa provident illum, quo quidem delectus id
          beatae laboriosam sed. Unde eaque impedit perspiciatis ex iure dolores
          sequi vel? Optio saepe assumenda repudiandae deserunt a illum
          accusamus asperiores nemo. Reiciendis accusantium recusandae
          laboriosam maxime ipsa id est, dolore inventore quis. Dolore ex, quis
          aliquid deserunt beatae quos quas consectetur reiciendis voluptatem
          corporis officiis delectus eaque illo explicabo inventore eius amet
          facere ea ullam optio quaerat deleniti non! Consequatur, officiis quo
        </p>
        <h2 className="text-6xl">About SuperEats</h2>
        <p className="text-2xl">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem
          eveniet optio consequatur reprehenderit quisquam voluptatum excepturi,
          rerum dicta tempora, ex ipsa provident illum, quo quidem delectus id
          beatae laboriosam sed. Unde eaque impedit perspiciatis ex iure dolores
          sequi vel? Optio saepe assumenda repudiandae deserunt a illum
          accusamus asperiores nemo. Reiciendis accusantium recusandae
          laboriosam maxime ipsa id est, dolore inventore quis. Dolore ex, quis
          aliquid deserunt beatae quos quas consectetur reiciendis voluptatem
          corporis officiis delectus eaque illo explicabo inventore eius amet
          facere ea ullam optio quaerat deleniti non! Consequatur, officiis quo.
        </p>
        <h2 className="text-6xl">About SuperEats</h2>
        <p className="text-2xl">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem
          eveniet optio consequatur reprehenderit quisquam voluptatum excepturi,
          rerum dicta tempora, ex ipsa provident illum, quo quidem delectus id
          beatae laboriosam sed. Unde eaque impedit perspiciatis ex iure dolores
          sequi vel? Optio saepe assumenda repudiandae deserunt a illum
          accusamus asperiores nemo. Reiciendis accusantium recusandae
          laboriosam maxime ipsa id est, dolore inventore quis. Dolore ex, quis
          aliquid deserunt beatae quos quas consectetur reiciendis voluptatem
          corporis officiis delectus eaque illo explicabo inventore eius amet
          facere ea ullam optio quaerat deleniti non! Consequatur, officiis quo.
        </p>
      </div>
    );
}


export default About;